...
case_TestIsBreed = runT $ do
  ...
  ask_ (atomic $ die) =<< unsafe_turtle 1
...
case_SetBreedToNonBreed = runT $ do
  atomic $ crt 1
  ask_ (atomic $ set_breed "turtles") =<< unsafe_turtle 0

  atomic $ crt 1
  ask_ (atomic $ set_breed "frogs") =<< unsafe_turtle 1

  atomic $ crt 1
  ask_ (atomic $ set_breed "patches") =<< unsafe_turtle 2

  atomic $ crt 1
  ask_ (atomic $ set_breed "links") =<< unsafe_turtle 3
