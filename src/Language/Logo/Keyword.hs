...
directed_link_breed [p,s] = do
  ...
  ss <- funD (mkName s) [clause [varP x, varP y] 
                        (normalB [| do (_, _, _) <- Reader.ask :: CSTM Context; ls <- lift $ readTVar __links; return [maybe Nobody (LinkRef ($(varE x),$(varE y))) $M.lookup ($(varE x),$(varE y)) ls] |]) []]
  return [sp, ss]
