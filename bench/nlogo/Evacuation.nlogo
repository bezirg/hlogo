...
to go
  if not any? pedestrians [print ticks print timer stop]
  ask pedestrians with [pcolor = lime] [die] ;; They exit
  ask pedestrians [execute-behaviour]
  tick
end
...
to-report free-neighbors
  report neighbors with [pcolor != blue and not any? other pedestrians-on self and exit-distance < [exit-distance] of myself]
end
@#$#@#$#@
GRAPHICS-WINDOW
368
